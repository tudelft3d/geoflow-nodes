...
typedef CGAL::cpp11::tuple<Point, Vector, int, bool, double, int> PNL;
...
int main(int argc, char *argv[])
{
  ...
  lasreadopener.set_file_name("/Users/ravi/git/heightjump-detect/build/ahn3.las");
  ...
  for (auto p : points){
    ...
    // if (jump_count >= 1 && jump_count <= 5){// ie we assume the point to be on an edge of this cluster
      // edge_points.push_back(p); // add this point to a new set of candidate points for region-growing line-fitting
      p.get<0>()=Point(p.get<0>().x(),p.get<0>().y(),0); // project to z=0 - seems to yield more reliable line detection
      edge_points.push_back(p); // add this point to a new set of candidate points for region-growing line-fitting
    }
  }
  // line fitting in set of candidate edgepoints
  // TODO!!: use a better line detector, eg based on region growing and one that updates the line for optimal fit during the region growing
  Efficient_ransac line_detection;
  Efficient_ransac::Parameters l_parameters;
  l_parameters.min_points = 22;
  l_parameters.epsilon = 0.30;
  parameters.probability = 0.01;
  l_parameters.normal_threshold = 0; 

  line_detection.set_input(edge_points);
  line_detection.add_shape_factory<SCLine>();
  line_detection.detect(l_parameters);
  std::cout << line_detection.shapes().end() - line_detection.shapes().begin() << " lines detected." << std::endl;
  std::vector<int> edges_index_map(edge_points.size(),-1); // property map that stores the line index for each point
  int line_id=0;
  for(auto lshape: line_detection.shapes()){
    SCLine* l = dynamic_cast<SCLine*>(lshape.get());
    std::cout << lshape->info() <<std::endl;
    //find the two extreme points on this line
    double minpl=1, maxpl=0;
    size_t j=0;
    for(auto lind : lshape->indices_of_assigned_points()){
      edges_index_map[lind] = line_id;
      Point this_p(edge_points[lind].get<0>());
      double pl = l->projection(this_p);
      if (j++==0){
        minpl = maxpl = pl;
      } else {
    ...
    line_id++;
    Point p0 = l->point_from_projection(minpl);
    Point p1 = l->point_from_projection(maxpl);
